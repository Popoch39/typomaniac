...
- **Replay** (`src/components/replay/`, route `/duels/$duelId`, fichier `duels_.$duelId.tsx` pour ne pas s'imbriquer dans `/duels`) : entièrement côté client, sans le store du Duel ni la WebSocket. `replay-clock.ts` (logique pure, testée) tient le temps de lecture t, play / pause ; `useReplayClock` le fait avancer à chaque frame sur l'horloge injectée (`ClockContext`). À chaque t, `runAt` du moteur rebâtit les deux Runs et `computeScore` les deux Scores, rendus par `RunText` comme en jeu.
