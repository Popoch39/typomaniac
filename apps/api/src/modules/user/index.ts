...
export const userModule = ({ auth, trustProxy, users, searchRateLimit }: UserModuleConfig) => {
    ...
    new Elysia({ name: "user", seed: users })
      ...
      .get(
        ...
        ({ user, query, set }) => {
          ...
          return searchUsers(users, { id: user.id, handle: user.handle ?? null }, query.handle);
        },
        {
          auth: true,
