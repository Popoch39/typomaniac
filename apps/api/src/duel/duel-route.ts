...
import { DuelQueue } from "./duel-queue";
import { clientMessage, ServerMessage } from "./protocol";

export type DuelRouteConfig = { auth: AuthHandler; trustProxy: boolean; clock: Clock };
...
export const duelRoute = ({ auth, trustProxy, clock }: DuelRouteConfig) => {
  const queue = new DuelQueue(clock);
