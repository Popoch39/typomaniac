...
- **Tests** : `betterAuth({ ...authOptions(…), database: memoryAdapter(…), plugins: [testUtils()] })`, puis `(await auth.$context).test.login({ userId })` pour obtenir le cookie d'une Session. Pour la connexion sociale et la liaison : des fournisseurs factices (`verifyIdToken` + `getUserInfo`) et `POST /api/auth/sign-in/social` avec `idToken`, qui passe par la vraie logique User / Account sans aller-retour OAuth.
