...
- Modules actuels : `auth` (instance Better Auth, macro `auth`), `me` (`/me`, `/me/pace`), `handle`, `user` (Users, recherche), `friend` (Friend requests, Friends), `duel`.
...
Plugins dans `src/plugins/`, montés par `createApp` dans cet ordre : request-id, request-logger, security-headers, cors, api-docs, error-handler, body-limit, rate-limit, authentication (puis les modules me, handle, user, friend, duel). L'error-handler doit rester avant les plugins qui rejettent des requêtes.
