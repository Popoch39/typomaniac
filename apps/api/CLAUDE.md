...
- **Duel en cours** : `RunningDuel` (`src/duel/running-duel.ts`), un par paire, gardé par `DuelQueue` pour chaque User jusqu'à ce qu'il le quitte une fois fini (`join-queue` ignoré avant la fin). Chaque Keystroke d'un lot passe par `acceptKeystroke` du moteur (arrivée = maintenant − `startsAt`, tolérance `END_TOLERANCE_MS` après la fin) : les acceptés sont relayés à l'adversaire (`opponent-keystrokes`) ; un seul rejet dans le lot renvoie au client `resync` (ses Keystrokes acceptés, le nombre reçu de lui, ceux de l'adversaire), le client garde ce qu'il a envoyé au-delà de ce nombre.
...
- **Horloge** : `AppConfig.clock` (`src/clock.ts`, `systemClock` en prod), une horloge déplacée à la main dans les tests (`now`).
